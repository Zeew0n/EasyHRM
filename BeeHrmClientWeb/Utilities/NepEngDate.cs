...
namespace BeeHrmClientWeb.Utilities
{
    public class NepEngDate
    {
        ...
        public static string EngToNep(DateTime dateTime)
        {
            if (IsRangeEng(dateTime.Year, dateTime.Month, dateTime.Day) == false)
            {
                return "0000-00-00";
            }
            else
            {
                return EnglishToNepaliDate(dateTime.Year, dateTime.Month, dateTime.Day);
            }


        }
        public static string EnglishToNepaliDate(int yy, int mm, int dd)
        {
            if (bs == null) InitializeData();
            ...
            string d2 = total_nDays.ToString();

            NepaliDate = y + "-" + m2.PadLeft(2, '0') + "-" + d2.PadLeft(2, '0');

            return NepaliDate;

        }

        public static string NepToEng(string nepDate)
        {
            string[] d = nepDate.Split('-');
            ...
            else
            {
                ...
                int yy = Convert.ToInt32(d[0]);
                int mm = Convert.ToInt32(d[1]);
                int dd = Convert.ToInt32(d[2]);
                ...
                else
                {
                    ...
                    string m2 = m.ToString();
                    string d2 = total_eDays.ToString();
                    var date = y + "-" + m2.PadLeft(2, '0') + "-" + d2.PadLeft(2, '0');
                    return date;
